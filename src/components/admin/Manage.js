...
import { Modal, Text, View, StyleSheet, ImageBackground, KeyboardAvoidingView, TouchableOpacity, ScrollView } from 'react-native';
...
class Manage extends Component {
  state = { email: '', password: '', newReqs: [], newReqPage: true, allRecords: [] };
  ...
  renderTitle() {
    if(this.state.newReqPage){
      return <Text style={{marginTop:20, marginLeft:15, fontWeight:'bold', fontSize:30}}>New Requests</Text>
    }
    else{
      return <Text style={{marginTop:20, marginLeft:15, fontWeight:'bold', fontSize:30}}>Handyman</Text>
    }
  }
    ...
    render(){
        return (          
            ...
            <View style={styles.buttonContainer}>
              <TouchableOpacity style={[styles.buttonStyle, this.state.newReqPage ? {backgroundColor: 'black'}: {}]} onPress={() => {
                this.setState({
                  newReqPage: true,

                })
              }}>
                <Text style={this.state.newReqPage ? {color: 'white'}: {}}>New requests</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.buttonStyle, , this.state.newReqPage ? {}: {backgroundColor: 'black'}]} onPress={() => {
                this.setState({
                  newReqPage: false
                ...
                <Text style={this.state.newReqPage ? {}: {color: 'white'}}>Handyman</Text>
              </TouchableOpacity>
            </View>
            <ScrollView>
              {this.renderTitle()}              

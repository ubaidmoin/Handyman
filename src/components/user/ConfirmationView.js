...
import { Text, View, StyleSheet, ImageBackground, Dimensions, TouchableOpacity, Image, ScrollView } from 'react-native';
...
class ConfirmationView extends Component {
    ...
    state = { isModalVisibleWork: false, isModalVisible: false, dialogVisible: false, coordinates: [], google_api_key: 'AIzaSyDNA9nURgVDmwVFFOatLwYS-mGtOsGelqY', latitude: null, longitude: null, error: null, coords: {latitude:33.5856, longitude:73.0675}, origin: null, services: [], handyman: {"name":"", "mobileno":"", "rating":"", "category":"", "coords": {"latitude":0 ,"longitude":0}}}
    ...
    render(){
        return (          
            ...
            propagateSwipe
            swipeDirection={["down"]}  
            onSwipeComplete={() => {this.setState({isModalVisibleWork:false})
            this.timeoutHandle = setTimeout(()=>{
            ...
            <View style={{ height:350, flexDirection: 'column', marginTop:180}}>   
              ...
              <TouchableOpacity style={{marginTop:18, marginLeft:'40%', backgroundColor:'grey', height:30, width:30, borderRadius:60, alignItems:'center', justifyContent:'center'}} onPress={() => {this.setState({isModalVisibleWork:false})
              this.timeoutHandle = setTimeout(()=>{
                this.setState({ dialogVisible: !this.state.dialogVisible })
              }, 5000);
            }}>
              ...
              <Card>
                ...
                <View style={styles.detailContainer}>
                    <Text style={{ fontSize: 20, fontWeight: 'bold'}}>{this.state.handyman.name}</Text>                            
                    <Text >{this.state.handyman.rating} ★</Text>
                    <Text style={{ fontWeight: 'bold'}}>{this.state.handyman.mobileno}</Text>   
                    <Text style={{ fontWeight: 'bold'}}>{this.state.handyman.category}</Text>                                                                                                                                                 
                </View>  
                </CardSection>
              </Card>                
              </View>               
